...
const RawLogViewer = () => {
    ...
    const filteredOptions = options.filter(o => !Object.values(filter).map(e => e.key).includes(o.value))
        ...
        <Card title={"Raw Terraform Log Viewer"}>
            <Flex vertical>
                <Flex vertical gap={20}>
                    {Array(count).fill(0).map((_, i) => (<Flex gap={20} key={i}>
                        <Select
                            value={filter[i]?.key}
                            onChange={val => {
                                setFilter(prev => ({
                                    ...prev, [i]: {...prev[i], key: val}
                                }))
                            }}
                        ...
                        <Input
                            ...
                            onChange={(e) => setFilter(prev => ({
                                ...prev, [i]: {...prev[i], value: e.target.value}
                            }))}
                            placeholder={'value'}
